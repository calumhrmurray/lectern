...
test.describe('bundled example decks', () => {
  for (const [id, count] of [['whale-evolution', 15], ['naturalisation-fr', 17]] as const) {
    test(`${id} opens from the welcome screen`, async ({ page }) => {
      await page.goto('/');
      await page.locator('.lec-list-item', { hasText: id === 'whale-evolution' ? 'From land to sea' : 'naturalisation' }).click();
