...
<section><h2>Five</h2><div hidden data-ai-note="" style="position:absolute;left:10px;top:20px;"><p data-by="author">draw a whale</p><p data-by="ai">Added a silhouette.</p><p data-by="author">make it bigger</p></div></section>
</div></div></body></html>`;
...
describe('notes for AI', () => {
  ...
  it('writes a prompt an assistant can act on', () => {
    ...
    expect(p).toContain('3 pending notes');
    expect(p).toContain('author: draw a whale → you (earlier): Added a silhouette. → author: make it bigger');
    expect(p).toContain('<p data-by="ai">');
  ...
  it('the template is hidden and marked, and keeps `hidden` through cleaning', () => {
    ...
    expect(el.querySelector('p[data-by="author"]')).not.toBeNull();
