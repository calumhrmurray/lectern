...
export function shortcutsDialog(): Promise<string> {
  ...
  return modal({
    ...
    body: h('div', { class: 'lec-shortcuts' },
      ...
      row('⏎', 'Edit the selected text'), row('Type', 'With a text object selected: start editing, replacing its text'), row('N', 'New note for AI, where the pointer is'), row('right-click', 'Note for AI, where you point'), row('+ / − / 0', 'Zoom in / out / fit (nothing selected)'),
